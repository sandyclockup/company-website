          ...
          <div class="col-sm-9 col-md-7 col-lg-5 mx-auto">
            <div class="card border-0 shadow rounded-3 my-5">
              <div class="card-body p-4 p-sm-5">
                ...
                <Form ref="form" @submit="handleRegister" :validation-schema="schema">


                  <div class="form-floating mb-3">
                    <Field name="username" v-slot="{ field, errors }">
                      <input v-bind="field" placeholder="Username" type="text" :readonly="loading" :class="errors.length == 0 ? 'form-control' : 'form-control is-invalid'">
                    </Field>
                  ...
                  <div class="form-check mb-3 input-form">
                    <Field name="acceptTerms" v-slot="{ field, errors }">
                      <input v-bind="field" type="checkbox" v-model="toggle" true-value="true"  false-value="false" :class="errors.length == 0 ? 'form-check-input' : 'form-check-input is-invalid'" />
                    </Field>
                    <label class="form-check-label" for="check">
                      <small>Creating an account means you’re okay with our
                        <router-link :to="{ name: 'Term'}" class="text-success">
                            Terms of Service
                        </router-link>
                            and
                        <router-link :to="{ name: 'Privacy'}" class="text-success">
                            Privacy Policy.
                        </router-link>
                      </small>
                    </label>
                    <ErrorMessage name="acceptTerms" class="error invalid-feedback" />
                  </div>
                  ...
                  <div class="d-grid">
                    <button type="submit" id="btn-form-submit" class="btn btn-success btn-login text-uppercase fw-bold" :disabled="loading">
                        ...
                        <span v-else>
                            <i class="fas fa-user-edit me-2"></i> Create Account
                        </span>
                    </button>
                  </div>
                  ...
                  <div class="mt-3 text-center">
                    <small>
                      Already a member?
                      <router-link :to="{ name: 'AuthLogin'}">Sign In</router-link>
                    </small>
                  </div>
    ...
    export default {
        ...
        mounted() {
            if(localStorage.getItem('config')){
              ...
              document.title = "Sign Up | " + config.config_site_title
            }
        },
        data(){
            const schema = yup.object().shape({
                ...
                acceptTerms: yup.boolean().required("The terms and conditions must be accepted.").oneOf([true], "The terms and conditions must be accepted.")
            });
            return {
                loading: false,
                message: "",
